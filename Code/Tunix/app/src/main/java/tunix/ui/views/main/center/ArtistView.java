...
public class ArtistView extends JPanel {
    ...
    // Promoted to field so updateFollowButton() can reach it
    ...
    // Called by ArtistController after follow/unfollow completes
    public void updateFollowButton(boolean following) {
        if (followButton != null) {
            followButton.setText(following ? "Following" : "Follow");
            followButton.setForeground(
                following ? new Color(0x1DB954) : new Color(0xAAAAAA)
            );
        }
    }
    ...
    private JPanel buildHeader() {
        ...
        header.add(buildArtistAvatar(), BorderLayout.WEST);
        header.add(buildArtistInfo(),   BorderLayout.CENTER);
    ...
    private JLabel buildArtistAvatar() {
        ...
        String initial = artist == null
                ? "A"
                : artist.getTitle().substring(0, 1).toUpperCase();
    ...
    private JPanel buildArtistInfo() {
        ...
        verified.setForeground(new Color(120, 180, 255));
        verified.setFont(new Font("Dialog", Font.BOLD, 13));

        JLabel artistName = new JLabel(
                artist == null ? "Unknown Artist" : artist.getTitle()
        );
        artistName.setForeground(Color.WHITE);
        artistName.setFont(new Font("Dialog", Font.BOLD, 42));

        JLabel followers = new JLabel(
                artist == null ? "0 followers" : artist.getFollowersCount() + " followers"
        );
        followers.setForeground(Color.LIGHT_GRAY);
        followers.setFont(new Font("Dialog", Font.PLAIN, 14));

        JLabel biography = new JLabel(
                artist == null
                        ? ""
        ...
        biography.setFont(new Font("Dialog", Font.PLAIN, 13));
    ...
    private JPanel buildActionsBar() {
        ...
        // Assign to field so updateFollowButton() can update it later
        followButton = buildActionButton("Follow");
        followButton.addActionListener(e -> {
            if (controller != null) controller.toggleFollow();
        });
    ...
    private JPanel buildPopularSongsSection() {
        ...
        JLabel title = new JLabel("Popular");
    ...
    private JScrollPane buildSongsTable() {
        ...
        String[]   columns = { "#", "Title", "Duration" };
        Object[][] rows    = new Object[topSongs.size()][3];
        ...
        for (int i = 0; i < topSongs.size(); i++) {
            ...
            rows[i][2]   = minutes + ":" + String.format("%02d", seconds);
        }
    ...
    private JPanel buildAlbumsSection() {
        ...
        JPanel albumsRow = new JPanel(new FlowLayout(FlowLayout.LEFT, 20, 0));
        albumsRow.setBackground(Color.DARK_GRAY);

        for (Album album : albums) {
            albumsRow.add(buildAlbumCard(album));
        }
    ...
    private JPanel buildAlbumCard(Album album) {
        ...
        JLabel cover    = buildAlbumCover();
        JLabel title    = new JLabel(album.getTitle());
        title.setForeground(Color.WHITE);
        title.setFont(new Font("Dialog", Font.BOLD, 14));

        JLabel subtitle = new JLabel(album.getSongs().size() + " songs");
    ...
    private JLabel buildAlbumCover() {
        ...
        g2.setColor(Color.WHITE);
        g2.setFont(new Font("Dialog", Font.PLAIN, 42));
